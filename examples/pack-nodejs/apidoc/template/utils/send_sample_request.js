define([
    ...
    function sendSampleRequest(group, name, version, type)
    {
        ...
        // add by augejs
        ['access-token', 'device-uuid', 'app-vendor' ].forEach((storeKey) => {
        ...
        function displaySuccess(data, status, jqXHR) {
          ...
          if (contentType.startsWith('image/')) {
            ...
            for (let i = 0; i < responseTextLen; ++i) {
              bytesStr += String.fromCharCode(responseText.charCodeAt(i) & 0xFF)
            }
            const responseBase64Text = window.btoa(bytesStr);

            console.log(responseBase64Text);

            outputContent = jsonResponse = '<img src="data:'+ contentType +';base64,' + responseBase64Text + '">';
            $root.find(".sample-request-response-json").html(outputContent);
          } else {
            try {
              outputContent = JSON.parse(responseText);
              outputContent = JSON.stringify(jsonResponse, null, 4);
            } catch (e) {
